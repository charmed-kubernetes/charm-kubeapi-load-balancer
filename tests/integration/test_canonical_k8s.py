...
async def test_build_and_deploy(ops_test, local_charm_path):
    ...
    bundle, *overlays = await ops_test.async_render_bundles(
        ops_test.Bundle("canonical-kubernetes", channel="edge"),
        Path("tests/data/k8s.yaml"),
        arch="amd64",
...
async def test_load_balancer_forced_address(ops_test):
    ...
    try:
        kcp = ops_test.model.applications["k8s"]
        action = await kcp.units[0].run_action("get-kubeconfig")
        result = await action.wait()
        assert f"https://{address}" in result.results["kubeconfig"]
    finally:
