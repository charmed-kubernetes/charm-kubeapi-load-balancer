...
def provide_lb_consumers():
    ...
    for request in lb_consumers.all_requests:
        ...
        if request.protocol not in (
            ...
            continue
        if lb_address:
            private_address = lb_address[0]
            public_address = lb_address[0]
        else:
...
def provide_application_details():
    ...
    if lb_address:
        website.configure(
            port=lb_port, private_address=lb_address[0], hostname=lb_address[0]
        )
    else:
